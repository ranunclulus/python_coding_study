...
# 가장 작은 물고기가 있는 좌표를 받았을 경우 물고기끼리 이동하는 함수
def swap_fish(fishes, min_x, min_y):
...
def can_eat(fishes, shark_x, shark_y):
    possible = []
...
def dfs(shark_x, shark_y, eat, fishes):
    global result # 전역 변수 result의 값을 변경하겠다
    fishes = copy.deepcopy(fishes)
    dead_fish[fishes[shark_x][shark_y][0]] = 1 # 먹은 물고기 표시
    # 물고기 먹기
    eat += fishes[shark_x][shark_y][0]
    fishes[shark_x][shark_y][0] = 0
    move_fish(fishes) # 물고기 이동
    possible = can_eat(fishes, shark_x, shark_y) # 먹을 수 있는 물고기 리스트 받아 오기
    if len(possible) == 0:
        result = max(result, eat)
        return
    else:
        for nx, ny in possible:
            dfs(nx, ny, eat, fishes)
